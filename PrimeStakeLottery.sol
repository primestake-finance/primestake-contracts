...
// File: contracts\PrimeStakeLotteryNFT.sol

pragma solidity 0.6.12;
pragma experimental ABIEncoderV2;

contract PrimeStakeLotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("PrimeStake Lottery Ticket", "STEAKLT") {}
...
// 4 numbers
contract PrimeStakeLottery is OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    IBEP20 public steakToken;
    // The Lottery NFT for tickets
    PrimeStakeLotteryNFT public lotteryNFT;
    // adminAddress
    ...
    function reset() external onlyAdmin {
        ...
        if (getMatchingRewardAmount(issueIndex - 1, 4) == 0) {
            uint256 amount = getTotalRewards(issueIndex - 1)
                .mul(allocation[0])
                .div(100);
    ...
    function internalBuy(uint256 _price, uint8[4] memory _numbers) internal {
        ...
        uint256 tokenId = lotteryNFT.newLotteryItem(
            address(this),
            _numbers,
            _price,
            issueIndex
    ...
    function _buySingleTicket(uint256 _price, uint8[4] memory _numbers)
        private
        returns (uint256)
    {
        for (uint256 i = 0; i < 4; i++) {
        ...
        totalAmount = totalAmount.add(_price);
        lastTimestamp = block.timestamp;
        uint32[winningCombinations]
            memory userCombinations = generateCombinations(_numbers);
        for (uint256 i = 0; i < winningCombinations; i++) {
            userBuyAmountSum[issueIndex][
                userCombinations[i]
            ] = userBuyAmountSum[issueIndex][userCombinations[i]].add(_price);
        }
        return tokenId;
    }
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        steakToken.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        steakToken.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            safeSteakTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            safeSteakTransfer(address(msg.sender), totalReward);
        }
        emit MultiClaim(msg.sender, totalReward);
    }

    function generateCombinations(uint8[4] memory number)
        public
        pure
        returns (uint32[winningCombinations] memory)
    {
        uint32 packedNumber = (number[0] << 24) +
            (number[1] << 16) +
            (number[2] << 8) +
            number[3];
    ...
    {
        uint32[winningCombinations] memory combinations = generateCombinations(
            winningNumbers
        );
        ...
        totalMatched3 = totalMatched3.add(
            userBuyAmountSum[issueIndex][combinations[2]]
        );
        totalMatched3 = totalMatched3.add(
            userBuyAmountSum[issueIndex][combinations[3]]
        );
        totalMatched3 = totalMatched3.add(
            userBuyAmountSum[issueIndex][combinations[4]]
        );
        ...
        totalMatched2 = totalMatched2.add(
            userBuyAmountSum[issueIndex][combinations[6]]
        );
        totalMatched2 = totalMatched2.add(
            userBuyAmountSum[issueIndex][combinations[7]]
        );
        totalMatched2 = totalMatched2.add(
            userBuyAmountSum[issueIndex][combinations[8]]
        );
        totalMatched2 = totalMatched2.add(
            userBuyAmountSum[issueIndex][combinations[9]]
        );
        totalMatched2 = totalMatched2.add(
            userBuyAmountSum[issueIndex][combinations[10]]
        );
    ...
    function getMatchingRewardAmount(
        ...
        require(
            _matchingNumber >= 2 && _matchingNumber <= 4,
            "getMatchingRewardAmount: INVALID"
        );
    ...
    function getRewardView(uint256 _tokenId) public view returns (uint256) {
        ...
        if (matchingNumber > 1) {
            ...
            uint256 poolAmount = getTotalRewards(_issueIndex)
                ...
                .div(100);
            reward = amount.mul(1e12).mul(poolAmount).div(
    ...
    function safeSteakTransfer(address _to, uint256 _amount) internal {
        uint256 steakBal = steakToken.balanceOf(address(this));
        if (_amount > steakBal) {
            steakToken.transfer(_to, steakBal);
        } else {
            steakToken.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        steakToken.safeTransfer(address(msg.sender), _amount);
